...
Vector3d calculate_light_intensity(std::vector<Light *> &lights, Vector3d &n,
    ...
    // // diffuse reflection
    // // Vector3d i_d = ((*light.get_intensity() * (*kd)) * std::max(l.dot(n),
    // 0.0));
    // // specular reflection
    // // Vector3d i_e = ((*light.get_intensity() * (*ke)) *
    // std::max(pow(v.dot(r), m), 0.0)); Vector3d i_l =
    // *light.get_contribution(reflex, l, n, v, r); Vector3d i_a =
    // *ambient_l.get_contribution(reflex, l, n, v, r);
