...
double Mesh::intersect(Vector3d p_0, Vector3d dr)
{
    ...
    // std::cout << "before render: " << vertex_list.at(4).toStr() << std::endl;
...
void Mesh::scale(Vector3d scale_factor)
{
    ...
    std::vector<Vector3d> matrix_scale = {
        Vector3d(scale_factor.x_, 0, 0, 0),
        Vector3d(0, scale_factor.y_, 0, 0),
        Vector3d(0, 0, scale_factor.z_, 0),
        Vector3d(0, 0, 0, 1)};

    std::vector<Vector3d> inverse_m = {
        Vector3d(1 / scale_factor.x_, 0, 0, 0),
    ...
    //               { p = p.mult_vector_matriz4d(inverse_m); });
}

void Mesh::translate(Vector3d target_position)
{
    std::vector<Vector3d> matrix_scale = {
        Vector3d(1, 0, 0, target_position.x_),
...
void Mesh::rotate(Axis rotation_axis, double angle)
{
    ...
    case Axis::X_AXIS:
        matrix_scale = {
            Vector3d(1, 0, 0, 0),
            Vector3d(0, cos_t, -sin_t, 0),
            Vector3d(0, sin_t, cos_t, 0),
            Vector3d(0, 0, 0, 1)};
        break;
    case Axis::Y_AXIS:
        matrix_scale = {
            Vector3d(cos_t, 0, sin_t, 0),
            Vector3d(0, 1, 0, 0),
    ...
    case Axis::Z_AXIS:
        matrix_scale = {
            Vector3d(cos_t, -sin_t, 0, 0),
            Vector3d(sin_t, cos_t, 0, 0),
            Vector3d(0, 0, 1, 0),
            Vector3d(0, 0, 0, 1)};
        break;
    }
...
void Mesh::shearing(ShearingTypes shearing_axis, double angle)
{
    ...
    // std::for_each(begin(matrix_scale), end(matrix_scale), [&](Vector3d &p)
    //               { std::cout << p.toStr() << std::endl; });
    // vector<Vector3d> m_t = Vector3d::calculate_transpose(matrix_scale);
    // std::cout << "Transposta: " << std::endl;
    // std::for_each(begin(m_t), end(m_t), [&](Vector3d &p)
    //               { std::cout << p.toStr() << std::endl; });

    // vector<Vector3d> m_i = Vector3d::calculate_inverse(m_t);
    // std::cout << "Inversa: " << std::endl;
    // std::for_each(begin(m_i), end(m_i), [&](Vector3d &p)
    //               { std::cout << p.toStr() << std::endl; });
    ...
    std::for_each(begin(normal_list) + 1, end(normal_list), [&](Vector3d &n)
                  { n = n.mult_vector_matriz4d(inverse_t); });
}
